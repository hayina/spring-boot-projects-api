...
//			marche.setDateLastModif(new Date());
		...
		// les marchesSocietes existantes vont plus �tre r�ferenc� alors ils seront supprim� grace � la properties orphanRemoval=true
		...
		if( isOs ) {
			...
			// on recup�re d'abord les os qui ont �t� supprim�s par UI on va les utiliser apres dans les attachements
...
//			marche.getMarchesOss().removeIf(os -> deletedOs.stream().anyMatch(del -> del.getId().equals(os.getId())));
			...
			if( ! osStartBean.typeOs.value.equals(api.enums.OsType.COMMENCEMENT.value) ) {
			...
			// check integrity os
			int i=0;
			Integer previousOsType = api.enums.OsType.REPRISE.value;
			...
			for(OsBean os : bean.os) {
				...
				if( i > 0 ) {
					...
					if(os.typeOs.value.equals(api.enums.OsType.ARRET.value)) {
						workDaysLastArretOrRecep += ChronoUnit.DAYS.between(Helpers.toLocalDate(lastReprise), Helpers.toLocalDate(os.dateOs));
					} else if(os.typeOs.value.equals(api.enums.OsType.REPRISE.value)) {
...
//					if( os.typeOs.value.equals(api.enums.OsType.COMMENCEMENT.value) ) { throw new OsIntegrityException(); }
				}
...
//				mOs.setDateOs(new SimpleDateFormat("dd/MM/yyyy").parse("01/09/2019"));
			...
			if( bean.dateReceptionProv != null ) {
				if(previousOsType.equals(api.enums.OsType.ARRET.value)) {
					throw new OsIntegrityException();
				}
...
//			marche.setLastReprise(lastReprise);
			...
			if(isTaux) {
				...
				if( OsCurrentTaux != null && OsCurrentTaux.typeOs.value.equals(api.enums.OsType.REPRISE.value) ) {
		...
		if( isTaux ) {
			...
			// On supprime les taux qui ont etait supprim�s par le user dans la page "Edit Marche"
...
//
		...
		if(isTaux) {
			...
			System.out.println("Le taux courant observé par l'agent sur terrain est de => (" + newCurrentTaux.valueTaux + " %) -> normalement il doit être  -> ("+currentTauxInDays+") jours");
			System.out.println("pour ce taux courant le nombres de jours effectués est de ("+workDaysCurrentTaux+") jours  -> normalement le taux doit être  (" + workPrecent + " %)");
...
//		List<Integer> marchesIds = marcheDao.getMarchesIdsByProjet(idProjet);
	...
	public  MarcheBean prepareMarcheDto(Integer idMarche) {
		...
		if( marche.getDateReceptionProvisoire() != null ) {
			...
			marcheDto.workDays = marche.getCurrentOs().getOsType().getId().equals(api.enums.OsType.ARRET.value) ?
